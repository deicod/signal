...
func TestCipherUsesPreKyberVersions(t *testing.T) {
	aliceID, err := signal.GenerateIdentityKeyPair()
	require.NoError(t, err)
	bobID, err := signal.GenerateIdentityKeyPair()
	require.NoError(t, err)
	...
	bundle, err := signal.BuildPreKeyBundle(bobStore, 1, &pre.ID, signed.ID, nil)
	require.NoError(t, err)
	...
	preKeyMsg, err := wire.ParsePreKeySignalMessage(initial)
	require.NoError(t, err)
	require.Equal(t, wire.CiphertextMessagePreKyberVersion, preKeyMsg.MessageVersion())
	require.NotNil(t, preKeyMsg.PreKeyID())
	require.Equal(t, pre.ID, *preKeyMsg.PreKeyID())
	require.Equal(t, signed.ID, preKeyMsg.SignedPreKeyID())
	require.Nil(t, preKeyMsg.KyberPreKeyID())
	require.Empty(t, preKeyMsg.KyberCiphertext())
	...
	signalMsg, err := wire.ParseSignalMessage(reply)
	require.NoError(t, err)
	require.Equal(t, wire.CiphertextMessagePreKyberVersion, signalMsg.MessageVersion())
